...
void CWorkingChangesFileListCtrl::Init(  DWORD dwColumns,  DWORD dwAllowedColumns,  const CString& sRegistryPrefix,  unsigned __int64 dwContextMenus,  bool bHasCheckboxes  )
{
  Locker lock(m_criticalSection);

  // Set default columns bit mask to columns specified, but ensure at least full path column is always shown 
  m_dwDefaultColumns = dwColumns | SIWC_COLUMN_FULLPATH;
  ...
  CString sFullRowSelect = _T("Software\\TortoiseSI\\") + sRegistryPrefix + _T("\\FullRowSelect");
  ...
  DWORD exStyle =
    ...
    LVS_EX_FULLROWSELECT;         // Selecting item causes all sub-items to be selected too (requires LVS_REPORT style)
  ...
  m_ColumnManager.ReadSettings(
    m_dwDefaultColumns, 
    SIWC_Columns::SIWC_COLUMN_ALL & ~(dwAllowedColumns | m_dwDefaultColumns), 
    sRegistryPrefix, 
    SIWC_NUMCOLUMNS);
...
// registry access
void ColumnManager::ReadSettings(DWORD dwDefaultColumns, DWORD dwHideColumns, const CString& containerName, int nColumns, int * columnWidths /* = NULL */)
{
  // Turn off hidden columns
  m_dwDefaultColumns = dwDefaultColumns & ~dwHideColumns;

  // Resize column vector to allow space for all columns
  m_columns.resize(nColumns);

  // Update column information
  for (int i = 0, p = 1; i < nColumns; ++i, p *= 2)
  {
    ...
    if (columnWidths == NULL) {
      ...
      m_columns[i].width = columnWidths[i];
    }

    m_columns[i].visible = true;
    m_columns[i].relevant = !(dwHideColumns & p);
  }
